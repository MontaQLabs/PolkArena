  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-sui-sea">🧠 Quiz</h1>
        ...
        <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-sui-sea hover:bg-sui-sea/90">
      ...
      {/* Join Room Section */}
      <Card className="mb-8 border-2 border-storm-200 hover:border-sui-sea transition-all duration-300">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <span className="text-sui-sea">🎯</span>
            Join Quiz Room
          </CardTitle>
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button onClick={joinRoom} disabled={joinPin.length !== 6} className="bg-sui-sea hover:bg-sui-sea/90">
              Join Room
            </Button>
          </div>
        </CardContent>
      ...
      {myQuizzes.length > 0 && (
        <div className="mb-8">
          <h2 className="text-2xl font-semibold mb-4 text-sui-sea">My Quizzes</h2>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {myQuizzes.map((quiz) => (
              <Card key={quiz.id} className="border-2 border-storm-200 hover:border-sui-sea transition-all duration-300 group">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <span className="text-sui-sea">🧠</span>
                    {quiz.title}
                ...
                <CardContent>
                  <div className="flex gap-2">
                    <Button asChild size="sm" className="bg-sui-sea hover:bg-sui-sea/90">
      ...
      {/* All Quizzes Section */}
      <div>
        <h2 className="text-2xl font-semibold mb-4 text-sui-sea">Available Quizzes</h2>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {quizzes.map((quiz) => (
            <Card key={quiz.id} className="border-2 border-storm-200 hover:border-sui-sea transition-all duration-300 group">
              <CardHeader>
